...
export default function Create() {
   ...
   useEffect(() => {
      ...
      if(urlParams.get('type')) {
         ...
         if(urlParams.get('type') == "abastecimento") getFuelSupply();
         if(urlParams.get('type') == "nota") getInvoice();
   ...
   return (<>
        ...
        <CardHeader className={styles.header} style={{justifyContent:"flex-start", alignItems: "center"}}>
           <IoArrowBackCircleSharp style={{width:"45px",height:"70px",color:"#009E8B",cursor:"pointer"}}
                                   onClick={() => {router.back()}}/>
           <h1 className={styles.header_h1}>Edição de {type == "abastecimento" && "Abastecimento"}
